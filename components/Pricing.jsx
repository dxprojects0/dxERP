...
const cardTone = (id) => {
    if (id === 'pro')
        return 'border-[color:var(--primary)]/40 bg-[color:var(--primary)]/5';
    if (id === 'business')
        return 'border-app bg-app/60';
    return 'border-app bg-surface';
};
const Pricing = ({ current = null, onSelect, }) => {
    ...
    return (<section className="space-y-3">
      ...
      <div className="space-y-3">
        {PLANS.map((plan) => {
            ...
            return (<article key={plan.id} className={`rounded-2xl border p-4 md:p-5 ${cardTone(plan.id)} smooth-transition`}>
              <button onClick={() => setExpanded((prev) => (prev === plan.id ? null : plan.id))} className="w-full text-left flex items-start justify-between gap-3">
                <div>
              ...
              {isOpen && (<div className="mt-4 pt-4 border-t border-app space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {plan.features.map((feature) => (<div key={feature.name} className={`rounded-lg border px-3 py-2 text-sm flex items-start gap-2 ${feature.available ? 'border-app' : 'border-app/60 opacity-70'}`}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => onSelect?.(plan.id)} className={`px-4 py-2 rounded-xl text-sm font-semibold ${isCurrent ? 'bg-black text-white' : 'bg-primary-app text-white'}`}>
