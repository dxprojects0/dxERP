...
import { applyRemoteProfile, hydrateConfigData, markAuthenticated, markSignedOut, setLastCloudSyncAt, } from '../features/configSlice';
import { getLatestSnapshot, getUserProfile, getUserProfileByEmail, observeAuthUser, observeUserWorkspace, saveUserProfile, } from '../utils/firebase';
...
import { deriveRoleFromEmail, isPaidPlan, normalizePlan } from '../utils/plans';
import { persistSnapshotByPlan } from '../utils/dataAccess';
import { ensureUserAccountDocument, observeUserPlanChanges } from '../utils/planEngine';
const AppRuntime = () => {
    const dispatch = useDispatch();
    const state = useSelector((s) => s);
    const syncTimer = useRef(null);
    const adminWriteTimer = useRef(null);
    const lastUserSyncHash = useRef('');
    const lastAdminWriteHash = useRef('');
    const storageModeRef = useRef('temporary');
    useEffect(() => {
        const unsub = observeAuthUser(async (user) => {
            try {
                ...
                const profile = await getUserProfile(user.uid) || await getUserProfileByEmail(user.email);
                if (!profile) {
                    dispatch(hydrateConfigData({
                        role: emailRole,
                        plan: emailRole === 'admin' ? 'business' : 'free',
                        onboardingCompleted: true,
                        selectedProfessionId: emailRole === 'admin' ? 'admin' : null,
                        customTools: TOOL_DEFINITIONS.map((item) => item.id),
                    }));
                    return;
                }
                const role = profile.role === 'admin' || emailRole === 'admin' ? 'admin' : 'user';
                const profilePlan = role === 'admin'
                ...
                const plan = normalizePlan(account?.plan || profilePlan);
                const onboardingCompleted = true;
                dispatch(applyRemoteProfile({
                    shopName: profile.shopName || null,
                    ownerName: profile.ownerName || null,
                    phoneNumber: profile.phoneNumber || null,
                    profileLocked: profile.profileLocked === true,
    ...
    useEffect(() => {
        const { config } = state;
        if (!config.isAuthenticated || !config.authUid)
            return;
    ...
    useEffect(() => {
        const { config } = state;
        if (!config.isAuthenticated || !config.authUid || !config.isAdmin)
            return;
    ...
    useEffect(() => {
        ...
        if (payloadHash === lastUserSyncHash.current)
            return;
    ...
    useEffect(() => {
        ...
        if (payloadHash === lastAdminWriteHash.current)
            return;
        lastAdminWriteHash.current = payloadHash;
        if (adminWriteTimer.current)
            window.clearTimeout(adminWriteTimer.current);
