...
export const ProtectedRoute = ({ children }) => {
    ...
    if (!isAuthenticated) {
        return (<div className="max-w-xl mx-auto bg-surface border border-app rounded-2xl p-6 text-center space-y-3">
...
export const AdminRoute = ({ children }) => {
    const { isAuthenticated, isAdmin, authResolved } = useSelector((state) => state.config);
    if (!authResolved)
        return null;
    if (!isAuthenticated)
