...
const Layout = () => {
    ...
    const [publicThemeMode, setPublicThemeMode] = useState(() => getPublicTheme());
    const location = useLocation();
    const [pageAnim, setPageAnim] = useState(false);
    const { isAuthenticated, isAdmin, themeMode, themePalette, customTools, plan, firstUseAt, lastLoginPromptDate, } = useSelector((state) => state.config);
    ...
    const selectedTools = useMemo(() => {
        ...
        return Array.from(new Set([...TOOL_DEFINITIONS.map((item) => item.id), ...allowedCustom]));
    }, [customTools, plan, isAdmin]);
    useEffect(() => {
    ...
    useEffect(() => {
        if (!isAppReady || isAdmin || isPaidPlan(plan) || !firstUseAt)
            return;
    ...
    return (<div className="min-h-screen bg-app text-app">
      <ToastHost />
      {isAppReady && (<aside className="hidden md:flex fixed left-0 top-0 h-full w-64 border-r border-app bg-surface z-40 flex-col">
          <button onClick={() => navigate(ROUTES.userDashboard)} className="h-20 border-b border-app text-left px-6">
            <p className="text-xs uppercase tracking-[0.2em] text-subtle py-1">
  Dashboard
        </p>

        <div className="flex items-center gap-2">
  <img src="/favicon.png" alt="DhandaX" className="h-6 w-6 rounded-md"/>
  ...
  <h1 className="text-2xl font-black text-subtle">
    DhandaX
      ...
      <div className={isAppReady ? 'md:ml-64 min-h-screen flex flex-col' : 'min-h-screen flex flex-col'}>
        <header className={`h-14 px-4 md:px-6 items-center justify-between sticky top-0 z-30 ${showTopHeader ? 'flex' : 'hidden'} ${(isPublicNavRoute && !isAppReady) ? '' : 'bg-surface border-b border-app'}`} style={(isPublicNavRoute && !isAppReady) ? { background: 'transparent', border: 'none', boxShadow: 'none', color: '#e5edff' } : {}}>
          <div className="flex items-center gap-3">
            {isAppReady && (<button onClick={() => setShowMenu(true)} className="md:hidden p-2 rounded-lg border border-app">
                <Menu size={16}/>
              </button>)}
            {isPublicNavRoute && !isAppReady ? (<div className="flex items-center gap-2">
          ...
          {isAppReady && <div />}
          {!isAppReady && isPublicNavRoute && (<button onClick={() => {
                const next = publicThemeMode === 'dark' ? 'light' : 'dark';
                setPublicThemeMode(next);
                setPublicTheme(next);
            }} className="inline-flex items-center gap-1 px-3 py-2 rounded-lg border border-app text-xs font-semibold" title="Toggle Light/Dark">
        ...
        {isAppReady && (<div className="md:hidden fixed bottom-0 left-0 right-0 bg-surface border-t border-app z-40 pb-[env(safe-area-inset-bottom)]">
            <div className="grid grid-cols-5 items-center">
              ...
              <div className="flex flex-col items-center justify-center">
                <button onClick={() => setShowToolMenu(true)} className="h-12 w-12 -mt-5 rounded-full bg-primary-app text-white border-4 border-surface flex items-center justify-center shadow-lg" title="Menu">
      ...
      {showMenu && isAppReady && (<div className="md:hidden fixed inset-0 bg-black/50 z-50">
      ...
      {showToolMenu && isAppReady && (<div className="md:hidden fixed inset-0 z-[80] bg-black/45" onClick={() => setShowToolMenu(false)}>
          <div className="absolute bottom-16 left-2 right-2 rounded-2xl border border-app bg-surface p-3 shadow-2xl max-h-[62vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {selectedTools.map((tool) => {
                ...
                return (<button key={tool} onClick={() => {
      ...
      {showUpgradePrompt && isAppReady && (<div className="fixed inset-0 z-[90] bg-black/45 p-4 flex items-center justify-center">
          <div className="w-full max-w-md bg-surface border border-app rounded-2xl p-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button onClick={closeUpgradePrompt} className="px-3 py-2 rounded-xl border border-app font-semibold text-sm">
                Later
              </button>
            </div>
          </div>
