...
const getStateKey = (email) => `root:${(email || GUEST_KEY).toLowerCase()}`;
export const setActiveEmailKey = (email) => {
    if (!email) {
...
export const loadState = async () => {
    try {
        ...
        if (activeEmail === GUEST_KEY)
            return undefined;
        if (activeEmail === 'admin@dxtoolz.com')
            return undefined;
...
export const clearStateForEmail = async (email) => {
    try {
        const db = await openDb();
        const tx = db.transaction(STORE_NAME, 'readwrite');
...
export const clearAllIndexedDbState = async () => {
    try {
