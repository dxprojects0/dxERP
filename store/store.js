...
export const createAppStore = (preloadedState) => {
    ...
    const store = configureStore({
        ...
        middleware: (getDefaultMiddleware) => getDefaultMiddleware().concat((_) => (next) => (action) => {
            const result = next(action);
            const type = action?.type;
            if (!type || type.startsWith('@@'))
                return result;
            const allowedSlices = ['inventory/', 'repairs/', 'config/', 'pos/', 'business/', 'finance/', 'customers/'];
            if (!allowedSlices.some((prefix) => type.startsWith(prefix)))
                return result;
            const excluded = [
                'markAuthenticated',
                'markSignedOut',
            ...
            if (excluded.some((word) => type.includes(word)))
                return result;
