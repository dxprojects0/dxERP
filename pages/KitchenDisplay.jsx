...
const KitchenDisplay = () => {
    ...
    const orders = useSelector((state) => state.business.kitchenOrders);
    ...
    const createOrder = () => {
        if (!tableNo.trim() || !itemsInput.trim())
            return;
    ...
    return (<div className="space-y-5 pb-20">
      ...
      <section className="bg-surface border border-app rounded-2xl p-4 grid grid-cols-1 md:grid-cols-[180px_1fr_160px] gap-2">
        <input className="border border-app rounded-xl px-3 py-2 bg-transparent" placeholder="Table no (e.g. 5)" value={tableNo} onChange={(e) => setTableNo(e.target.value)}/>
        <input className="border border-app rounded-xl px-3 py-2 bg-transparent" placeholder="Items separated by comma (Burger, Fries, Coffee)" value={itemsInput} onChange={(e) => setItemsInput(e.target.value)}/>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {orders.map((order) => {
            ...
            return (<article key={order.id} className="bg-surface border border-app rounded-2xl p-4 space-y-3">
              ...
              <div className="border border-app rounded-xl p-3 space-y-1">
                {order.items.map((item, idx) => (<div key={`${item.name}-${idx}`} className="text-sm flex justify-between">
                    <span>{item.name}</span>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex gap-2">
                  {order.status === 'New' && (<button onClick={() => dispatch(updateKitchenStatus({ id: order.id, status: 'Preparing' }))} className="px-2.5 py-1.5 rounded-lg bg-amber-500 text-white text-xs font-semibold inline-flex items-center gap-1">
                      <Flame size={12}/> Preparing
                    </button>)}
                  {order.status === 'Preparing' && (<button onClick={() => dispatch(updateKitchenStatus({ id: order.id, status: 'Ready' }))} className="px-2.5 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-semibold inline-flex items-center gap-1">
                      <CheckCircle size={12}/> Ready
                    </button>)}
                  {order.status === 'Ready' && (<button onClick={() => setConfirmDeliverId(order.id)} className="px-2.5 py-1.5 rounded-lg bg-slate-900 text-white text-xs font-semibold">
                      Deliver
                    </button>)}
                  {order.status === 'Delivered' && (<button onClick={() => dispatch(deleteKitchenOrder(order.id))} className="px-2.5 py-1.5 rounded-lg border border-[var(--danger)] text-[color:var(--danger)] text-xs font-semibold inline-flex items-center gap-1">
      ...
      {orders.length === 0 && (<div className="p-10 text-center text-subtle bg-surface border border-dashed border-app rounded-2xl">
          No kitchen orders yet.
        </div>)}

      {confirmDeliverId && (<Modal isOpen={Boolean(confirmDeliverId)} onClose={() => setConfirmDeliverId(null)} title="Confirm Delivery" maxWidth="24rem" closeOnBackdrop={false}>
