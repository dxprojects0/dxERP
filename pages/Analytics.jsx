...
const toMonth = (date) => date.toISOString().slice(0, 7);
...
const Analytics = () => {
    ...
    const monthInvoices = useMemo(() => (invoices || []).filter((inv) => {
        if (!inv.date?.startsWith(selectedMonth))
            return false;
        if (selectedDate && !inv.date.startsWith(selectedDate))
            return false;
        return true;
      ...
      <section className="bg-surface border border-app rounded-2xl p-4 md:p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[220px_220px_auto] gap-2 mt-4">
          <label className="text-sm">
            ...
            <input type="month" min="2026-01" value={monthInput} onChange={(e) => setMonthInput(e.target.value)} className="mt-1 w-full border border-app rounded-xl px-3 py-2 bg-transparent"/>
          </label>
          <label className="text-sm">
            <span className="text-xs text-subtle uppercase">Specific Date (optional)</span>
            <input type="date" min={`${monthInput || '2026-01'}-01`} value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="mt-1 w-full border border-app rounded-xl px-3 py-2 bg-transparent"/>
          </label>
          <div className="flex items-end">
            <button onClick={() => setSelectedMonth(monthInput || currentMonth)} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary-app text-white font-semibold">
      ...
      <section className="bg-surface border border-app rounded-2xl p-4 md:p-5">
        ...
        <div className="mt-4 space-y-2">
          {summary.monthSeries.map(([month, row]) => {
            ...
            return (<div key={month} className="space-y-1">
      ...
      <section className="bg-surface border border-app rounded-2xl p-4 md:p-5">
        ...
        <div className="mt-4 overflow-x-auto pb-2">
          <div className="flex flex-col gap-3">
            ...
            return (<div key={date + idx} className="heatmap-cell" data-level={level} title={`${date} • ₹ ${Math.round(value)}`}/>);
        })}
              </div>
            ...
            <div className="flex items-center gap-2 text-[11px] text-subtle">
              {Array.from(new Set(contribution.cells.map((c) => c.date.slice(0, 7)))).map((month) => (<span key={month} className="min-w-[38px] text-center border border-app rounded-full px-2 py-1 bg-app/40">
                  {month.slice(5, 7)}
                </span>))}
            </div>
          </div>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div className="bg-surface border border-app rounded-2xl p-4 md:p-5">
          ...
          <div className="mt-3 space-y-2 max-h-[50vh] overflow-y-auto pr-0.5">
            {summary.topProducts.map((item) => (<div key={item.name} className="rounded-xl border border-app px-3 py-2">
        ...
        <div className="bg-surface border border-app rounded-2xl p-4 md:p-5">
          ...
          <div className="mt-3 space-y-2 max-h-[50vh] overflow-y-auto pr-0.5">
            {summary.topCategories.map((item) => (<div key={item.name} className="rounded-xl border border-app px-3 py-2 flex justify-between items-center gap-2">
