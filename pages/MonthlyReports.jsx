...
const MonthlyReports = () => {
    ...
    const { plan } = useSelector((state) => state.config);
    ...
    const monthlyInvoices = useMemo(() => (invoices || []).filter((invoice) => invoice.date?.startsWith(selectedMonth)), [selectedMonth, invoices]);
    const monthTx = useMemo(() => (transactions || []).filter((tx) => tx.date?.startsWith(selectedMonth)), [selectedMonth, transactions]);
    ...
    return (<div className="max-w-5xl mx-auto space-y-6">
      <div className="bg-surface border border-app rounded-2xl p-5 flex flex-wrap items-end justify-between gap-3">
        ...
        <div>
          ...
          <input type="month" value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="block border border-app rounded-lg px-3 py-2 bg-transparent"/>
        </div>
      </div>
