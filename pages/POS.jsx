...
const POS = () => {
    ...
    const shopName = useSelector((state) => state.config.shopName);
    const profession = useSelector((state) => state.config.selectedProfessionId);
    ...
    const buildReceiptText = (invoice) => {
        const itemRows = invoice.items.map((i) => `${i.name} x${i.quantity} @ ₹ ${i.price}`).join('\n');
    ...
    const downloadPdf = () => {
        ...
        `)
            .join('');
        const printed = printHtmlDocument(`Invoice ${lastInvoice.id}`, `
        <style>
    ...
    return (<div className="space-y-5 overflow-x-hidden pb-24 md:pb-8">
      <div className="grid grid-cols-1 xl:grid-cols-[1.2fr_0.8fr] gap-5 items-start">
        <div className="bg-surface rounded-3xl border border-app flex flex-col overflow-hidden min-w-0">
          <div className="p-4 border-b border-app flex items-center gap-2 min-w-0">
            ...
            {!isMobile && (<button onClick={() => setShowCartActions((v) => !v)} className="h-9 w-9 rounded-xl border border-app text-sm">
                ...
              </button>)}
          </div>
          {!isMobile && showCartActions && (<div className="px-4 pt-3 pb-1 flex gap-2">
          ...
          <div className="p-4 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 overflow-y-auto overflow-x-hidden max-h-[48vh] md:max-h-[58vh]">
            {(filteredProducts || []).map((p) => (<button key={p.id} onClick={() => addToCart(p.id)} className="border border-app bg-surface rounded-2xl p-3 text-left hover:border-[var(--primary)] hover:bg-[color:var(--primary)]/10 transition-all">
                ...
                <div className="mt-2">
                  {editingProductId === p.id ? (<div className="flex items-center gap-1">
                      <input type="number" min="0" value={editingPrice} onClick={(e) => e.stopPropagation()} onChange={(e) => setEditingPrice(e.target.value)} className="w-full border border-app rounded-lg px-2 py-1 text-xs bg-transparent" placeholder="New price"/>
        ...
        <div className="bg-surface rounded-3xl border border-app flex flex-col min-w-0">
          <div className="p-4 border-b border-app flex items-center justify-between">
            ...
            {isMobile && (<button onClick={() => setShowAddProduct(true)} className="h-8 w-8 rounded-full bg-primary-app text-white flex items-center justify-center">
                <Plus size={16}/>
              </button>)}
          </div>
          ...
          <div className="p-4 space-y-2 max-h-[35vh] md:max-h-[250px] overflow-y-auto overflow-x-hidden">
            {(cartItems || []).map((item) => (<div key={item.id} className="flex justify-between items-center gap-2 p-2 border border-app rounded-xl">
                <div className="flex items-center gap-2">
                  {!isMobile && (<input type="checkbox" checked={selectedCartIds.includes(item.id)} onChange={(e) => setSelectedCartIds((prev) => e.target.checked ? [...prev, item.id] : prev.filter((id) => id !== item.id))}/>)}
                  <div>
          ...
          <div className="p-4 border-t border-app space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {['Cash', 'UPI', 'Card', 'Split'].map((mode) => (<button key={mode} onClick={() => setPaymentMode(mode)} type="button" className={`border rounded-xl p-2 text-xs font-bold ${paymentMode === mode ? 'border-[var(--primary)] text-primary-app bg-[color:var(--primary)]/10' : 'border-app text-subtle'}`}>
                  {mode}
                </button>))}
            </div>
      ...
      {showAddProduct && (<Modal isOpen={showAddProduct} onClose={() => setShowAddProduct(false)} title="Add Product" maxWidth="30rem" closeOnBackdrop>
      ...
      {showSuccess && lastInvoice && (<Modal isOpen={showSuccess} onClose={() => setShowSuccess(false)} title="Bill Completed" maxWidth="32rem" closeOnBackdrop>
