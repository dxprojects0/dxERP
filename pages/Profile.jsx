...
import { BarChart3, Crown, LogOut, Palette, Pencil } from 'lucide-react';
import { useDispatch, useSelector } from 'react-redux';
import { setOwnerName, setPhoneNumber, setPlan, setProfession, setShopName, setThemeMode, setThemePalette, signOutUser } from '../features/configSlice';
import { PALETTES } from '../utils/themes';
import { useNavigate } from 'react-router-dom';
import { getUserProfile, saveEditableProfileFields, savePaidInterestUser, savePlanAndPhone, signOutFirebase } from '../utils/firebase';
...
const Profile = () => {
    ...
    const { shopName, ownerName, phoneNumber, authEmail, authDisplayName, selectedProfessionId, plan, themeMode, themePalette, isAdmin, authUid, role, onboardingCompleted, } = useSelector((state) => state.config);
    ...
    const [editShop, setEditShop] = useState(shopName || '');
    const [editPhone, setEditPhone] = useState(phoneNumber || '');
    const [editCategory, setEditCategory] = useState(selectedProfessionId || '');
    ...
    const openEditModal = () => {
        ...
        setEditShop(shopName || '');
        setEditPhone(phoneNumber || '');
        setEditCategory(selectedProfessionId || '');
    ...
    const handleSaveProfile = async () => {
        ...
        if (!editOwner.trim() || !editShop.trim()) {
            emitToast({ variant: 'error', message: 'Owner and business name are required.' });
            return;
        }
        setSavingProfile(true);
        try {
            await saveEditableProfileFields(authUid, {
                email: authEmail,
                displayName: authDisplayName,
                ownerName: editOwner.trim(),
                shopName: editShop.trim(),
                phoneNumber: editPhone.trim(),
                selectedProfessionId: editCategory || null,
            });
            dispatch(setOwnerName(editOwner.trim()));
            dispatch(setShopName(editShop.trim()));
            dispatch(setPhoneNumber(editPhone.trim()));
            if (editCategory) {
                dispatch(setProfession(editCategory));
            }
            setShowEditModal(false);
            emitToast({ variant: 'success', message: 'Profile updated.' });
        }
        catch (error) {
            emitToast({ variant: 'error', message: error?.message || 'Could not update profile.' });
        }
        finally {
            setSavingProfile(false);
        }
    };
    const persistPlanSelection = async (nextPlan, nextPhone) => {
        if (!authUid)
    ...
    const submitPaidInterest = async (requestedPlan, submittedPhone) => {
        if (!authUid)
            throw new Error('Please login again.');
        const finalPhone = String(submittedPhone || phoneNumber || '').trim();
        if (!finalPhone)
            throw new Error('Update phone first');
    ...
    const handlePlanSelect = async (selected) => {
    ...
    return (<div className="max-w-5xl mx-auto space-y-6 pb-16">
      {completionPercent < 100 && !dismissCompletionHint && (<section className="bg-[#fffaf0] border border-[#ffd89b] rounded-2xl p-4">
          <div className="flex items-start justify-between gap-2">
            ...
            <button onClick={() => setDismissCompletionHint(true)} className="text-xs border border-[#ffd89b] rounded-lg px-2 py-1 text-[#7c4a03]">
              Later
            </button>
          </div>
      ...
      <section className="bg-surface border border-app rounded-2xl p-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-subtle mt-1">You can update your business details anytime.</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={openEditModal} className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-app text-xs font-semibold">
              <Pencil size={13}/> Edit
            </button>
      ...
      <Modal isOpen={showEditModal} onClose={() => setShowEditModal(false)} title="Edit profile" maxWidth="28rem">
        <div className="space-y-3">
          <div>
            <p className="text-xs text-subtle mb-1">Owner name</p>
            <input value={editOwner} onChange={(e) => setEditOwner(e.target.value)} className="w-full border border-app rounded-lg px-3 py-2 bg-transparent"/>
          </div>
          <div>
            <p className="text-xs text-subtle mb-1">Business name</p>
            <input value={editShop} onChange={(e) => setEditShop(e.target.value)} className="w-full border border-app rounded-lg px-3 py-2 bg-transparent"/>
          </div>
          <div>
            <p className="text-xs text-subtle mb-1">Phone number</p>
            <input value={editPhone} maxLength={10} inputMode="numeric" onChange={(e) => setEditPhone(e.target.value.replace(/\D/g, '').slice(0, 10))} className="w-full border border-app rounded-lg px-3 py-2 bg-transparent"/>
          </div>
          <div>
            <p className="text-xs text-subtle mb-1">Category</p>
            <select value={editCategory} onChange={(e) => setEditCategory(e.target.value)} className="w-full border border-app rounded-lg px-3 py-2 bg-transparent">
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            <button onClick={handleSaveProfile} disabled={savingProfile} className="px-3 py-2 rounded-lg bg-primary-app text-white font-semibold disabled:opacity-60">
              {savingProfile ? 'Saving...' : 'Save Changes'}
            </button>
            <button onClick={() => setShowEditModal(false)} className="px-3 py-2 rounded-lg border border-app font-semibold">
              Cancel
            </button>
          </div>
        </div>
      ...
      <section className="bg-surface border border-app rounded-2xl p-6">
        ...
        <div className="mt-4 inline-flex rounded-xl border border-app p-1 bg-app/60">
          <button onClick={() => { dispatch(setThemeMode('light')); emitToast({ variant: 'success', message: 'Theme switched to light.' }); }} className={`px-4 py-2 rounded-lg text-sm font-semibold ${themeMode === 'light' ? 'bg-primary-app text-white' : 'text-subtle'}`}>
            Light
          </button>
          <button onClick={() => { dispatch(setThemeMode('dark')); emitToast({ variant: 'success', message: 'Theme switched to dark.' }); }} className={`px-4 py-2 rounded-lg text-sm font-semibold ${themeMode === 'dark' ? 'bg-primary-app text-white' : 'text-subtle'}`}>
            Dark
          </button>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-4">
          {PALETTES.map((palette) => (<button key={palette.name} onClick={() => { dispatch(setThemePalette(palette.name)); emitToast({ variant: 'success', message: `Palette changed to ${palette.name}.` }); }} className={`rounded-xl border p-3 text-left ${themePalette === palette.name ? 'border-[var(--primary)] bg-[color:var(--primary)]/8' : 'border-app'}`}>
      ...
      <section className="bg-surface border border-app rounded-2xl p-6">
        ...
        <div className="mt-4">
          <Pricing current={normalizePlan(plan)} onSelect={handlePlanSelect}/>
          {!isAdmin && (<p className="text-xs text-subtle mt-2">
      ...
      {isAdmin && (<section className="bg-surface border border-app rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <button onClick={() => navigate(ROUTES.adminAnalytics)} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-primary-app text-white font-semibold">
