...
const SupplierOrders = () => {
    ...
    const markReceived = (id, itemId, qty) => {
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {orders.map((order) => (<tr key={order.id}>
                ...
                <td className="p-3">
                  {order.status === 'Pending' ? (<button className="px-2 py-1 bg-green-100 text-green-700 rounded text-xs" onClick={() => markReceived(order.id, order.linkedItemId, order.receivedQty)}>
                      Mark Received
                    </button>) : (<span className="text-xs text-slate-400">Completed</span>)}
                </td>
              </tr>))}
