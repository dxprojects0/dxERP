...
const StaffRoster = () => {
    ...
    const shifts = useSelector((state) => state.business.staffShifts);
    const [form, setForm] = useState({ staffName: '', date: '', shift: 'Morning' });
    const addShift = (e) => {
        e.preventDefault();
        if (!form.staffName.trim() || !form.date)
            return;
        dispatch(addStaffShift({
    ...
    return (<div className="space-y-5 pb-20">
      ...
      <form onSubmit={addShift} className="bg-surface border border-app rounded-2xl p-4 grid grid-cols-1 md:grid-cols-4 gap-2">
        ...
        <select className="border border-app rounded-xl px-3 py-2 bg-transparent" value={form.shift} onChange={(e) => setForm({ ...form, shift: e.target.value })}>
      ...
      <div className="hidden md:block bg-surface border border-app rounded-2xl p-4 overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody>
            {shifts.map((shift) => (<tr key={shift.id} className="border-t border-app">
                <td className="p-2">{shift.staffName}</td>
                <td className="p-2">{shift.date}</td>
                <td className="p-2">{shift.shift}</td>
                <td className="p-2">
                  <select value={shift.attendance} onChange={(e) => dispatch(updateStaffShift({ ...shift, attendance: e.target.value }))} className="border border-app rounded-lg p-1 text-sm bg-transparent">
      ...
      <div className="md:hidden space-y-2">
        {shifts.map((shift) => (<div key={shift.id} className="bg-surface border border-app rounded-xl p-3 space-y-2">
            <p className="font-semibold text-sm">{shift.staffName}</p>
            <p className="text-xs text-subtle">{shift.date} • {shift.shift}</p>
            <div className="grid grid-cols-2 gap-2">
              <select value={shift.attendance} onChange={(e) => dispatch(updateStaffShift({ ...shift, attendance: e.target.value }))} className="border border-app rounded-lg px-2 py-1.5 text-sm bg-transparent">
                <option>Present</option>
                <option>Absent</option>
              </select>
