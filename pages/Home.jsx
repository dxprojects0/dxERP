...
import { deriveRoleFromEmail } from '../utils/plans';
import { markAuthenticated } from '../features/configSlice';
...
const Home = () => {
    ...
    const { isAuthenticated, onboardingCompleted, isAdmin } = useSelector((state) => state.config);
    ...
    const handleStartFree = async () => {
        ...
        try {
            ...
            emitToast({ variant: 'success', message: 'Signed in successfully.' });
            navigate(ROUTES.userDashboard, { replace: true });
        }
        catch (error) {
            emitToast({ variant: 'error', message: error?.message || 'Google login failed.' });
        }
        finally {
            setLoading(false);
        }
    ...
    return (<div className="w-full min-h-[80vh] overflow-hidden relative flex items-center justify-center">
      ...
      <div className="w-full max-w-7xl px-5 md:px-10 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        ...
        <div className="text-center lg:text-left space-y-6 text-white">
          ...
          <div className="flex flex-wrap justify-center lg:justify-start gap-3">
            <button onClick={handleStartFree} disabled={loading} className="px-6 py-3 rounded-full bg-gradient-to-r from-blue-600 to-sky-500 font-semibold shadow-xl hover:scale-105 transition disabled:opacity-60">
              <Sparkles size={16} className="inline mr-2"/>
              {loading ? 'Signing In...' : 'Start Free'}
            </button>

            <button onClick={() => navigate(ROUTES.admin)} className="px-6 py-3 rounded-full border border-white/20 bg-white/10 backdrop-blur">
              Admin Login
            </button>
          </div>
        </div>
        ...
        <div className="flex justify-center relative">
          ...
          <div className="w-[2600px] sm:w-[530px] md:w-[580px] h-[400px] md:h-[560px] relative rounded-2xl overflow-hidden border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl">
            ...
            <div className="relative flex items-end justify-between h-full px-3 pb-3 gap-1">
              ...
              {bars.map((h, i) => (<div key={i} className="relative flex-1 rounded-t-md transition-all duration-700 ease-out" style={{
                height: `${h}%`,
          ...
          <div className="hidden md:block">
            {FEATURES.slice(0, 6).map((f, i) => (<div key={f} className="absolute text-[10px] px-2 py-1 rounded-full bg-white/10 backdrop-blur border border-white/20 text-blue-100" style={{
