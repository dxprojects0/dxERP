...
import { ArrowDownRight, ArrowUpRight, CircleDollarSign, BarChart3, Package, BookOpenText, Truck, CalendarDays, HeartPulse, CookingPot, Users, Wrench, ShieldCheck, ClipboardList, ReceiptIndianRupee, Plus, Settings2, } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const repairState = useSelector((state) => state.repairs);
    ...
    const { customTools, ownerName, shopName, plan, isAdmin, authUid, authEmail, } = useSelector((state) => state.config);
    const finalTools = Array.from(new Set([
        ...TOOL_DEFINITIONS.map((item) => item.id),
        ...(isPaidPlan(plan) ? customTools : []),
    ]));
    ...
    const hourlySales = React.useMemo(() => {
        ...
        invoices.forEach((inv) => {
            ...
            if (!Number.isNaN(hour) && hour >= 0 && hour < 24)
                buckets[hour] += inv.total || 0;
        });
        const max = Math.max(...buckets, 1);
        return { buckets, max };
    }, [invoices]);
    const useCountUp = (value, from = 0, duration = 700) => {
    ...
    const [activityView, setActivityView] = React.useState('24h');
    ...
    return (<div className="space-y-6 pb-24 md:pb-10">
      <section className="rounded-3xl border border-app bg-surface p-6">
        <h1 className="text-3xl font-black mt-1">Welcome{ownerName ? `, ${ownerName}` : ''}</h1>
        {shopName ? <p className="text-lg font-semibold mt-1">{shopName}</p> : null}
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {isAdmin && (<button onClick={checkConnectionNow} className="px-4 py-2 rounded-lg border border-app text-sm font-semibold">
              Check Connection
            </button>)}
          {isAdmin && (<button onClick={() => setShowSyncConfirm(true)} className="px-4 py-2 rounded-lg bg-primary-app text-white text-sm font-semibold">
              Sync Now (Admin)
            </button>)}
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-app bg-surface p-5">
          <div className="flex items-center justify-between gap-2 mb-3">
            ...
            <div className="inline-flex rounded-xl border border-app overflow-hidden text-xs font-semibold">
              <button className={`px-3 py-1 ${activityView === '24h' ? 'bg-primary-app text-white' : 'text-subtle'}`} onClick={() => setActivityView('24h')}>
                24h
              </button>
              <button className={`px-3 py-1 ${activityView === '7d' ? 'bg-primary-app text-white' : 'text-subtle'}`} onClick={() => setActivityView('7d')}>
                7d
              </button>
            </div>
          </div>

          {activityView === '24h' && (<div className="flex flex-col gap-4 overflow-x-auto pb-2">
              <div className="heatmap-grid heatmap-24">
                ...
                return (<div key={hour} className="heatmap-cell" data-level={level} title={`${hour}:00 • ₹ ${Math.round(value)}`}/>);
            })}
              </div>
              <div className="heatmap-legend">
                <span>Less</span>
                {[0, 1, 2, 3, 4].map((l) => (<span key={l} className="heatmap-cell" data-level={l} style={{ width: 14, height: 14 }}/>))}
                <span>More</span>
              </div>
            </div>)}

          {activityView === '7d' && (<div className="flex flex-col gap-4">
        ...
        <div className="rounded-2xl border border-app bg-surface p-5">
          ...
          <div className="space-y-2">
            {invoices.slice(-6).reverse().map((invoice) => (<div key={invoice.id} className="flex items-center justify-between rounded-xl border border-app px-3 py-2">
      ...
      <section className="rounded-2xl border border-app bg-surface p-5">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => navigate(ROUTES.userTools)} className="px-3 py-2 rounded-lg border border-app text-sm inline-flex items-center gap-1">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
          {finalTools.map((tool) => {
            ...
            return (<button key={tool} onClick={() => navigate(buildUserToolRoute(tool))} className="rounded-2xl border border-app px-4 py-4 min-h-[130px] text-left bg-app/35 hover:bg-[color:var(--primary)]/8 transition-all">
      ...
      {showSyncConfirm && isAdmin && (<Modal isOpen={showSyncConfirm} onClose={() => setShowSyncConfirm(false)} title="Save Data to Cloud" maxWidth="24rem" closeOnBackdrop={false}>
          <div className="space-y-3">
