...
const ExpiryTracking = () => {
    ...
    const returns = useSelector((state) => state.business.expiryReturns);
    const now = new Date();
    const withAlerts = useMemo(() => {
        return items
            .filter((i) => i.expiryDate)
            .map((i) => {
            const diffDays = Math.ceil((new Date(i.expiryDate).getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
            let alert = '';
            if (diffDays <= 30)
                alert = '30 days';
            else if (diffDays <= 60)
    ...
    const logReturn = (item) => {
        dispatch(addExpiryReturn({
            id: `RET-${Date.now()}`,
            itemName: item.name,
      ...
      <div className="bg-white border rounded-2xl overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {withAlerts.map((item) => (<tr key={item.id} className={item.expired ? 'bg-red-50' : ''}>
                <td className="p-3">{item.name}</td>
                <td className="p-3">{item.batchNo || '-'}</td>
                <td className="p-3">{item.expiryDate}</td>
                <td className="p-3">
                  {item.expired ? (<span className="text-xs px-2 py-1 rounded bg-red-100 text-red-700 font-bold">Expired</span>) : item.alert ? (<span className="text-xs px-2 py-1 rounded bg-amber-100 text-amber-700 font-bold">Alert {item.alert}</span>) : (<span className="text-xs text-slate-400">Healthy</span>)}
                </td>
                <td className="p-3">
                  {(item.expired || item.alert) && (<button className="px-2 py-1 rounded text-xs bg-blue-50 text-primary" onClick={() => logReturn(item)}>
                      Return to Supplier
                    </button>)}
                </td>
              </tr>))}
