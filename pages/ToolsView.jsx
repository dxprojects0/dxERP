...
const ToolsView = () => {
    ...
    const { customTools, plan } = useSelector((state) => state.config);
    const activeTools = Array.from(new Set([
    ...
    return (<div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 pb-20">
        {(activeTools || []).map((tool) => {
            ...
            return (<button key={tool} onClick={() => navigate(toolRouteMap[tool])} className="bg-surface p-5 rounded-xl border border-app text-left hover:shadow-md transition-all">
