...
const DailyReports = () => {
    ...
    const modeTotals = todayInvoices.reduce((acc, inv) => {
    ...
    const downloadReport = () => {
        ...
        printHtmlDocument(`Daily Report ${today}`, `
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white border rounded-2xl p-4">
          ...
          <div className="space-y-2 text-sm">
            {['Cash', 'UPI', 'Card', 'Split'].map((m) => (<div key={m} className="flex justify-between"><span>{m}</span><span className="font-bold">₹ {modeTotals[m] || 0}</span></div>))}
          </div>
        </div>
        <div className="bg-white border rounded-2xl p-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            {topItems.map(([name, qty]) => (<div key={name} className="flex justify-between"><span>{name}</span><span className="font-bold">{qty}</span></div>))}
          </div>
        </div>
      </div>
    </div>);
