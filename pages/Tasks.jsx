...
const Tasks = () => {
    ...
    const tasks = useSelector((state) => state.config.tasks);
    const [newTask, setNewTask] = useState('');
    const completedCount = tasks.filter((task) => task.done).length;
    return (<div className="max-w-4xl mx-auto space-y-6 pb-24 md:pb-20 overflow-x-hidden">
      ...
      <section className="rounded-[1rem] border border-app bg-surface p-3">
        <div className="flex gap-2 mb-4">
          <input value={newTask} onChange={(e) => setNewTask(e.target.value)} placeholder="Add a task..." className="flex-1 border border-app bg-transparent rounded-xl px-3 py-2 outline-none"/>
          <button onClick={() => {
            if (!newTask.trim())
                return;
        ...
        <div className="space-y-3 max-h-[62vh] overflow-y-auto overflow-x-hidden pr-0.5">
          {tasks.map((task) => (<div key={task.id} className="rounded-2xl border border-app px-4 py-3 flex items-center gap-3">
              ...
              <button onClick={() => dispatch(deleteTask(task.id))} className="px-3 py-1.5 rounded-lg text-white bg-red-600/80 border-2 border-red-500 font-semibold inline-flex items-center gap-1">
