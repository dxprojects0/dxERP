...
const HealthRecords = () => {
    ...
    const records = useSelector((state) => state.business.healthRecords);
    ...
    const filtered = useMemo(() => {
        ...
        if (!token)
            return sorted;
        return sorted.filter((record) => [record.patientName, record.patientPhone, record.diagnosisNotes].some((field) => String(field || '').toLowerCase().includes(token)));
    }, [records, query]);
    const openCreate = () => {
        setEditingId(null);
        setForm(blankForm);
    ...
    const printRecord = (record) => {
        printHtmlDocument(`Health Record ${record.id}`, `
    ...
    return (<div className="space-y-5 pb-20">
      ...
      <section className="bg-surface border border-app rounded-2xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          <button onClick={openCreate} className="h-10 w-10 rounded-xl bg-primary-app text-white flex items-center justify-center" title="Add new health record">
            <Plus size={16}/>
          </button>
          <div className="flex-1 border border-app rounded-xl px-3 py-2 flex items-center gap-2">
            <Search size={15} className="text-subtle"/>
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by patient name or phone number" className="w-full bg-transparent outline-none text-sm"/>
          </div>
        </div>

        <div className="space-y-2 max-h-[62vh] overflow-y-auto">
          {filtered.map((record) => (<button key={record.id} onClick={() => {
      ...
      {showFormModal && (<Modal isOpen={showFormModal} onClose={() => setShowFormModal(false)} title={editingId ? 'Update Health Record' : 'Add Health Record'} maxWidth="34rem" closeOnBackdrop={false}>
      ...
      {showDetailModal && activeRecord && (<Modal isOpen={showDetailModal} onClose={() => setShowDetailModal(false)} title="Patient Details" maxWidth="34rem" closeOnBackdrop>
