...
const STAGES = ['Received', 'Diagnose', 'In Process', 'Ready', 'Delivered'];
const RepairJobs = () => {
    const dispatch = useDispatch();
    const tickets = useSelector((state) => state.repairs.tickets);
    ...
    const addPartToTicket = (ticket, partId) => {
    ...
    const updateTicketPayment = (ticket, mode) => {
        dispatch(updateTicket({
            ...ticket,
            paid: !ticket.paid,
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white border rounded-xl p-3 flex items-center gap-2">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by ID, customer, phone, model, or IMEI" className="w-full outline-none text-sm"/>
      </div>

      <div className="flex gap-4 overflow-x-auto pb-4">
        {STAGES.map((stage) => (<div key={stage} className="min-w-[320px] bg-slate-50 border border-slate-200 rounded-2xl p-3">
            ...
            <div className="space-y-3">
              {filteredTickets
                ...
                .map((job) => (<div key={job.id} className="bg-white border rounded-xl p-3 space-y-2">
                    ...
                    <select className="w-full border rounded p-1 text-sm" value={job.stage} onChange={(e) => dispatch(updateStage({ id: job.id, stage: e.target.value }))}>
                      {STAGES.map((s) => (<option key={s} value={s}>{s}</option>))}
                    </select>

                    {(job.stage === 'Diagnose' || job.stage === 'Diagnosing') && (<div className="space-y-1">
                        <input className="w-full border rounded p-1 text-sm" placeholder="Diagnosis notes" value={job.diagnosisNotes || ''} onChange={(e) => dispatch(updateTicket({ ...job, diagnosisNotes: e.target.value }))}/>
                        <input type="number" className="w-full border rounded p-1 text-sm" placeholder="Estimated cost" value={job.estimatedCost} onChange={(e) => dispatch(updateTicket({ ...job, estimatedCost: Number(e.target.value) || 0 }))}/>
                    ...
                    {job.stage === 'In Process' && (<div className="space-y-1">
                        <input className="w-full border rounded p-1 text-sm" placeholder="Technician name" value={job.technician || ''} onChange={(e) => dispatch(updateTicket({ ...job, technician: e.target.value }))}/>
                        <div className="flex gap-1">
                          ...
                          <input type="number" min="1" className="w-16 border rounded p-1 text-sm" value={partQty[job.id] || 1} onChange={(e) => setPartQty((prev) => ({ ...prev, [job.id]: Number(e.target.value) || 1 }))}/>
                        </div>
                      </div>)}

                    {job.stage === 'Ready' && (<div className="space-y-1">
                        <input type="number" className="w-full border rounded p-1 text-sm" placeholder="Final amount" value={job.finalAmount || job.estimatedCost} onChange={(e) => dispatch(updateTicket({ ...job, finalAmount: Number(e.target.value) || 0 }))}/>
                        <select className="w-full border rounded p-1 text-sm" value={job.paymentMode || 'Cash'} onChange={(e) => dispatch(updateTicket({ ...job, paymentMode: e.target.value }))}>
                          <option>Cash</option>
                          <option>UPI</option>
                          <option>Card</option>
                          <option>Split</option>
      ...
      <Drawer title="Create Repair Job" isOpen={isOpen} onClose={() => setIsOpen(false)}>
        <form onSubmit={createJob} className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            {(Object.entries(newJob.accessories) || []).map(([name, checked]) => (<label key={name} className="flex items-center gap-1">
