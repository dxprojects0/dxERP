...
const formatRequestedPlanLabel = (plan) => {
...
const AdminAnalytics = () => {
    ...
    const filteredUsers = useMemo(() => {
        ...
        if (!token)
            return data.users;
        return data.users.filter((user) => [user.email, user.ownerName, user.shopName, user.category, user.plan].some((item) => String(item || '').toLowerCase().includes(token)));
    }, [data, query]);
    return (<div className="max-w-6xl mx-auto space-y-5 pb-24">
      <section className="bg-surface border border-app rounded-2xl p-5 space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <button onClick={load} disabled={loading} className="px-4 py-2 rounded-xl bg-primary-app text-white font-semibold inline-flex items-center gap-2 disabled:opacity-60">
        ...
        <div className="flex items-center gap-2 border border-app rounded-xl px-3 py-2">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search business, email, owner, category..." className="w-full bg-transparent outline-none text-sm"/>
        </div>
      </section>
      ...
      {data && (<>
          ...
          <section className="bg-surface border border-app rounded-2xl p-4 md:p-5 space-y-3">
            ...
            <div className="space-y-2 max-h-[55vh] overflow-y-auto">
              {filteredUsers.map((user) => (<div key={user.id} className="border border-app rounded-xl p-3">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <select value={user.plan || 'free'} disabled={String(user.email || '').toLowerCase() === 'admin@dxtoolz.com'} onChange={async (e) => {
          ...
          <section className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div className="bg-surface border border-app rounded-2xl p-4 md:p-5 space-y-2">
              ...
              <div className="max-h-[38vh] overflow-y-auto space-y-2">
                {(data.paidInterestUsers || []).map((entry) => (<div key={String(entry.requestId || entry.id)} className="border border-app rounded-xl p-3">
            ...
            <div className="bg-surface border border-app rounded-2xl p-4 md:p-5 space-y-2">
              ...
              <div className="max-h-[38vh] overflow-y-auto space-y-2">
                {(data.paidUsersSummary || []).map((entry) => (<div key={String(entry.uid || entry.id)} className="border border-app rounded-xl p-3">
