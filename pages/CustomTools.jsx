...
const CustomTools = () => {
    const { customTools, plan, isAdmin } = useSelector((state) => state.config);
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const categories = useMemo(() => Array.from(new Set(TOOL_DEFINITIONS.map((tool) => tool.category))), []);
    ...
    return (<div className="max-w-5xl mx-auto py-6 space-y-8">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-black text-slate-900">Add More Tools</h1>
          <p className="text-slate-500 mt-2 text-sm">Choose extra tools for your workspace.</p>
        </div>
        <button onClick={() => navigate(ROUTES.userDashboard)} disabled={customTools.length === 0} className={`px-5 py-3 rounded-xl text-sm font-semibold ${customTools.length ? 'bg-slate-900 text-white' : 'bg-slate-200 text-slate-400'}`}>
          Launch My Workspace
        </button>
      </div>

      {(categories || []).map((category) => (<section key={category} className="space-y-3">
