...
const Bills = () => {
    ...
    const finance = useSelector((state) => state.finance.transactions || []);
    const ledger = useSelector((state) => state.business.ledger || []);
    const shopName = useSelector((state) => state.config.shopName || 'Your Business');
    const [query, setQuery] = useState('');
    const [kindFilter, setKindFilter] = useState('all');
    const [active, setActive] = useState(null);
    const transactions = useMemo(() => {
        const billItems = (invoices || []).map((inv) => ({
            id: inv.id,
            kind: 'invoice',
            date: inv.date,
        ...
        const financeItems = (finance || []).map((tx) => ({
            id: tx.id,
    ...
    return (<div className="max-w-6xl mx-auto space-y-4 pb-24 overflow-x-hidden">
      <section className="bg-surface border border-app rounded-2xl p-4 md:p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_220px] gap-2">
          <div className="flex items-center gap-2 border border-app rounded-xl px-3 py-2 bg-app/40">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search invoice, customer, item, amount..." className="w-full min-w-0 bg-transparent outline-none text-sm"/>
          </div>
          <select value={kindFilter} onChange={(e) => setKindFilter(e.target.value)} className="border border-app rounded-xl px-3 py-2 bg-transparent text-sm">
      ...
      <section className="space-y-2 max-h-[68vh] overflow-y-auto overflow-x-hidden pr-0.5">
        {filtered.map((tx) => (<button key={`${tx.kind}-${tx.id}`} onClick={() => setActive(tx)} className="w-full bg-surface border border-app rounded-xl px-4 py-3 text-left flex items-center justify-between gap-3">
        ...
        {filtered.length === 0 && (<div className="bg-surface border border-app rounded-xl p-5 text-sm text-subtle text-center">
            No matching transactions.
          </div>)}
      </section>

      {active && (<Modal isOpen={Boolean(active)} onClose={() => setActive(null)} title="Transaction Details" maxWidth="56rem" closeOnBackdrop>
          <div className="space-y-3">
            ...
            {'items' in active.meta && Array.isArray(active.meta.items) && (<div className="rounded-xl border border-app p-3">
                <p className="font-semibold text-sm mb-2">Items</p>
                <div className="space-y-1">
                  {active.meta.items.map((item, idx) => (<div key={`${item.name || 'item'}-${idx}`} className="text-sm text-subtle flex justify-between gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              {active.kind === 'invoice' && (<button className="px-4 py-2 rounded-lg bg-primary-app text-white font-semibold" onClick={() => {
              ...
              <button className="px-4 py-2 rounded-lg border border-app text-foreground font-semibold" onClick={async () => {
                const meta = active.meta || {};
                const summary = [
