...
const Appointments = () => {
    ...
    const filtered = useMemo(() => {
        ...
        if (!token)
            return appointments;
        return appointments.filter((appointment) => [appointment.patientName, appointment.patientPhone, appointment.date, appointment.time].some((field) => String(field || '').toLowerCase().includes(token)));
    }, [appointments, search]);
    const handleAdd = (e) => {
        e.preventDefault();
    ...
    return (<div className="space-y-5 pb-20">
      ...
      <section className="bg-surface border border-app rounded-2xl p-4 space-y-3">
        <div className="flex items-center gap-2 border border-app rounded-xl px-3 py-2">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by patient name or phone" className="w-full bg-transparent outline-none text-sm"/>
        </div>

        <div className="space-y-2 max-h-[62vh] overflow-y-auto">
          {filtered.map((appointment) => (<div key={appointment.id} className="border border-app rounded-xl p-3 space-y-2">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`text-xs px-2 py-1 rounded-full border ${appointment.status === 'Booked'
                ? 'bg-blue-50 text-blue-700 border-blue-100'
                : appointment.status === 'Visited'
                    ? 'bg-emerald-50 text-emerald-700 border-emerald-100'
                    : 'bg-red-50 text-red-700 border-red-100'}`}>{appointment.status}</span>
              </div>
              <p className="text-xs text-subtle">{appointment.date} • {appointment.time}</p>
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => dispatch(updateAppointmentStatus({ id: appointment.id, status: 'Visited' }))} className="px-3 py-2 rounded-lg bg-emerald-600 text-white text-sm font-semibold inline-flex items-center justify-center gap-1">
                  <Check size={13}/> Visited
                </button>
                <button onClick={() => dispatch(updateAppointmentStatus({ id: appointment.id, status: 'Cancelled' }))} className="px-3 py-2 rounded-lg border border-[var(--danger)] text-[color:var(--danger)] text-sm font-semibold inline-flex items-center justify-center gap-1">
                  <X size={13}/> Cancel
                </button>
              </div>
            </div>))}
